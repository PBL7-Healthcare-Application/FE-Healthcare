...
import { setDoctorSchedule } from '../../../stores/doctor/DoctorThunk'
...
const SetUpSchedule = () => {
    ...
    const [isDisabled, setIsDisabled] = useState(profile?.workingTimeStart !== null);
    const dispatch = useDispatch()
    const [api, contextHolder] = notification.useNotification();
    const navigate = useNavigate();
    const handleFinish = (values) => {
        dispatch(setDoctorSchedule({
    ...
    return (
        <div className="setting-tab2">
            ...
            <div>
                ...
                >
                    {contextHolder}
                    <div style={{ width: '100%', display: 'flex', justifyContent: 'center', marginBottom: 20 }} > <span className="setting-font" style={{ fontSize: 25, fontWeight: 600, color: "#185FA0" }}>Set Up Your Schedule</span></div>
                    ...
                    >
                        <Select style={{ height: 40, marginTop: 10 }} options={Per.map((item) => ({ label: item.value, value: item.value }))} defaultValue={profile?.durationPerAppointment} disabled={isDisabled} />
                    </Form.Item>
                    {/* ================= */}
                    ...
                    <Form.Item
                        ...
                        normalize={(value) => value.trim()}
                    >
                        <Select style={{ height: 40, marginTop: 10 }} options={Time.map((item) => ({ label: item.value, value: item.value }))} defaultValue={profile?.workingTimeStart} disabled={isDisabled} />
                    </Form.Item>
                    {/* ================= */}
                    ...
                    <Form.Item
                        ...
                        rules={[
                            ({ getFieldValue }) => ({
                                validator(_, value) {
                                    ...
                                    return Promise.reject(new Error('Working time end must be greater than working time start'));
                                },
                            }),
                            {
                                required: true,
                        ...
                        normalize={(value) => value.trim()}
                    >
                        <Select style={{ height: 40, marginTop: 10 }} options={Time.map((item) => ({ label: item.value, value: item.value }))} defaultValue={profile?.workingTimeEnd} disabled={isDisabled} />
